...
contract PublicLoginContract is PublicPrivateAccessToken,PublicLoginContract_Interface{
    ...
    constructor(string memory _Pass ,string memory _URI)PublicPrivateAccessToken(_Pass,_URI){
    ...
    function User_Login(string memory _enterPass)public view OnlyToken returns(bool){
        if(keccak256(abi.encodePacked(ViewData())) == keccak256(abi.encodePacked(keccak256(abi.encodePacked(_enterPass))))){
            return true;
        } else{
            return false;
        }
    ...
    function CheckUserCreds(address _user)public view returns(bool){
        if(PublicPrivateAccessToken.OnlyIf(_user,1) == true){
            return true;
        } else {
            return false;
        }
