...
contract PublicPrivateAccessToken is ERC1155{
    bytes32 private PrivateData;
    uint Token =0;
    uint totalNFTs=5;
    ...
    constructor(string memory _PrivateData,string memory URI)ERC1155(URI){
        PrivateData = keccak256(abi.encodePacked(_PrivateData));
        _mint(msg.sender, Token, totalNFTs, "");
    }
    ...
    function ViewData() public multi_Token view returns(bytes32){
        return PrivateData;
    }
    function PrivateViewData()internal view returns(bytes32){
        return PrivateData;
    }
    ...
    function Edit(string memory _newData)public multi_Token returns(bool){
         PrivateData = keccak256(abi.encodePacked(_newData));
         return true;
    }
